...
var run = function run(params) {
  ...
  _logger2.default.logDebug({ message: 'package bundler run ' + JSON.stringify(args.type) });
  ...
  return new Promise(function (resolve, reject) {
    _utils2.default.getGitTag(branch).then(function (tag) {
      ...
      _logger2.default.logDebug({ message: 'Project version @' + project.version });
      return _utils2.default.createArchive(args.branch, project.version, 'git');
    }).then(function () {
      _logger2.default.logDebug({ message: 'Successfully packaged ' + project.name + ' (' + project.version + ') to EB' });
      resolve();
    }).fail(function (error) {
      _logger2.default.logDebug({ message: error });
      reject(error);
      process.exit(1);
    });
  });
