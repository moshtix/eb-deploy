...
var createArchive = function createArchive(branch, version, type) {
  ...
  return new Promise(function (resolve, reject) {
    _logger2.default.logDebug({ message: 'Creating project archive @ from branch ' + branch + ' ' + _path2.default.join(process.cwd(), 'release.' + version + '.zip') });

    exec('rm release.zip', function (err) {
      if (err) {
        _logger2.default.logDebug({ message: err });
      } else {
        _logger2.default.logDebug({ message: 'removed existing zip file' });
      }
    });
    ...
    if (type === 'git') {
      ...
      exec('git archive -o ' + fileName + ' ' + branch, function (err) {
        ...
        _logger2.default.logDebug({ message: 'Zip created' + fileName });
        resolve();
      });
    }
  });
